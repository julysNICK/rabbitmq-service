...
func main() {
	...
	// consumer.Listen([]string{"post.created", "post.deleted", "post.updated"}) is for listening to messages published to the exchange (we'll use it for receiving messages)
	err = consumer.Listen([]string{"post.created", "post.deleted", "post.updated"})
	if err != nil {
		panic(err)
	}
}
...
func connect() (*amqp.Connection, error) {
	...
	for {
		...
		if err != nil {
			log.Println("could not connect to rabbitmq, retrying...")
			counts++
		} else {
		...
		if counts > 10 {
			fmt.Println("could not connect to rabbitmq after 10 tries, exiting...")
			return nil, err
		}

		backOff = time.Duration(math.Pow(2, float64(counts))) * time.Second

		log.Printf("waiting %v seconds before trying again", backOff.Seconds())
